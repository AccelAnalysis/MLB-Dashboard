  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-slate-200">
      <table className="min-w-full divide-y divide-slate-200 text-sm">
        ...
        <tbody className="divide-y divide-slate-100 bg-white">{records.map((record, index) => <tr key={record.id || index}>{columns.map(([key]) => <td key={key} className="whitespace-nowrap px-4 py-3 text-slate-700">{currencyKeys.has(key) ? formatMetricValue(record[key], 'currency', false) : String(record[key] ?? '—')}</td>)}<td className="whitespace-nowrap px-4 py-3 text-right">{record.projectId && onOpenProject && <button type="button" onClick={() => onOpenProject(record.projectId)} className="font-bold text-blue-700 hover:underline">Open File</button>}</td></tr>)}</tbody>
      </table>
    </div>
  );
}
